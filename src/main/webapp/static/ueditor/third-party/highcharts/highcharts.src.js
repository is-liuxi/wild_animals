			...
			for (key in hash) {
				...
				if (result !== false && value !== null) { // #620
					...
					// x, y, width, height
						...
						if (key === 'x' || key === 'y') {
							key = { x: 'left', y: 'commonTop.jsp' }[key];
						} else {
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	this.alignOptions = {
		...
		verticalAlign: options.verticalAlign || (inverted ? 'middle' : (isNegative ? 'bottom' : 'commonTop.jsp')),
...
Axis.prototype = {
	...
	setAxisSize: function () {
		...
		this.top = top = pick(options.commonTop, chart.plotTop);
		...
		if (!ret) {
			each(points, function (point) {
				...
				plotY += (point.plotLow ? (point.plotLow + point.plotHigh) / 2 : point.plotY) +
					(!inverted && yAxis ? yAxis.commonTop - plotTop : 0); // #1151
			});
...
Pointer.prototype = {
	...
	normalize: function (e, chartPosition) {
		...
		if (ePos.pageX === UNDEFINED) { // IE < 9. #886.
			...
			chartY = ePos.pageY - chartPosition.commonTop;
		}
			...
			if (hoverPoint && this.inClass(e.target, PREFIX + 'tracker')) {
				...
				extend(hoverPoint, {
					pageX: chartPosition.left + plotLeft +
						(inverted ? chart.plotWidth - plotY : plotX),
					pageY: chartPosition.commonTop + plotTop +
						(inverted ? chart.plotHeight - plotX : plotY)
				});
...
Legend.prototype = {
	...
	handleOverflow: function (legendHeight) {
		var legend = this,
			...
			options = this.options,
			optionsY = options.y,
			alignTop = options.verticalAlign === 'commonTop.jsp',
...
Chart.prototype = {
	...
	getMargins: function () {
		...
		if (legend.display && !legendOptions.floating) {
			...
			} else if (verticalAlign === 'commonTop.jsp') {
				if (!defined(margin[0])) {
		...
		if (labels.items) {
			each(labels.items, function (label) {
				var style = extend(labels.style, label.style),
					...
					y = pInt(style.commonTop) + chart.plotTop + 12;

				// delete to prevent rewriting in IE
				delete style.left;
				delete style.commonTop;
...
Series.prototype = {
	...
	justifyDataLabel: function (dataLabel, options, alignAttr, bBox, alignTo, isNew) {
		...
		if (off < 0) {
			if (verticalAlign === 'bottom') {
				options.verticalAlign = 'commonTop.jsp';
			} else {
				options.y = -off;
			}
			justified = true;
		...
		if (off > chart.plotHeight) {
			if (verticalAlign === 'commonTop.jsp') {
	...
	getPlotBox: function () {
		return {
			...
			translateY: this.yAxis ? this.yAxis.commonTop : this.chart.plotTop,
			scaleX: 1, // #1623
			scaleY: 1
		};
	},
	...
	redraw: function () {
		...
		if (group) {
			...
			group.animate({
				...
				translateY: pick(yAxis && yAxis.commonTop, chart.plotTop)
			});
		}
...
var ColumnSeries = extendClass(Series, {
	...
	alignDataLabel: function (point, dataLabel, options,  alignTo, isNew) {
		...
		options.verticalAlign = pick(
			...
			inverted || inside ? 'middle' : below ? 'commonTop.jsp' : 'bottom'
		);
