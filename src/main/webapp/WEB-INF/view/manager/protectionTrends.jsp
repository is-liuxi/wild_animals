...
<div class="container">
    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal"> 添加保护动态</button>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">添加物种保护动态</h4>
            </div>
            <%-- 模态框内容处 --%>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="addProtectionTrend()">保存</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">修改物种保护动态</h4>
            </div>
            <%-- 模态框内容处 --%>
            <div class="modal-body" id="modalContent">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" onclick="updateProtectionTrend()">保存修改</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(function () {
        $.ajax({
            type: "GET",
            url: "${APP_PATH}/protectionTrends/list",
    ...
    function tableContent(data) {
        ...
        $.each(data, function (index, item) {
            ...
            content += '<td>';
            content += '<a class="btn btn-primary active" role="button" data-toggle="modal" data-target="#updateModal" onclick="selectProtectionTrendById(' + item.id + ')"> 修改 </a> ';
            content += ' <a href="javascript:void(0)" class="btn btn-danger active" role="button" onclick="deleteProtectionTrend(' + item.id + ')"> 删除 </a>';
            content += '</td>';
            content += '</tr>';
        });
    ...
    /* 修改前，查询内容回显 */
    function selectProtectionTrendById(id) {
        $("#id").val(id);
        $.ajax({
            type: "GET",
            url: "${APP_PATH}/protectionTrends/selectProtectionTrendsById/" + id,
            success: function (result) {
                var data = result.data;
    ...
    /* 修改物种保护动态 */
    function updateProtectionTrend() {
        var id = $("#id").val();
        ...
        $.ajax({
            type: "POST",
            url: "${APP_PATH}/protectionTrends/addProtectionTrend",
            ...
            success: function (result) {
                ...
                if (status === 200) {
                    ...
                    window.location.href = "${APP_PATH}/protectionTrends";
                } else {
    ...
    /* 删除 */
    function deleteProtectionTrend(id) {
        layer.confirm('是否删除物种保护动态：' + id, {
                //按钮
                btn: ['取消', '确定']
            }, function () {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "${APP_PATH}/protectionTrends/deleteProtectionTrend/" + id,
                    success: function (result) {
                        var data = result.data;
                        if (data === 200) {
